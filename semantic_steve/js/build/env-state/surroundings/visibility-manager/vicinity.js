...
exports.getVicinitiesToOffsets = getVicinitiesToOffsets;
 ...
 *
 */
function classifyVicinityOfPosition(pos, botPos, immediateSurroundingsRadius, distantSurroundingsRadius) {
    const distanceToPos = botPos.distanceTo(pos);
    if (distanceToPos <= immediateSurroundingsRadius) {
        return common_1.VicinityName.IMMEDIATE_SURROUNDINGS;
    }
        ...
        if (horizontalDist <= immediateSurroundingsRadius) {
            ...
            return pos.y > botPos.y
                ? common_1.VicinityName.DISTANT_SURROUNDINGS_UP
                : common_1.VicinityName.DISTANT_SURROUNDINGS_DOWN;
        }
        ...
        if (angle < 22.5 || angle >= 337.5)
            return common_1.VicinityName.DISTANT_SURROUNDINGS_NORTH;
        if (angle < 67.5)
            return common_1.VicinityName.DISTANT_SURROUNDINGS_NORTHEAST;
        if (angle < 112.5)
            return common_1.VicinityName.DISTANT_SURROUNDINGS_EAST;
        if (angle < 157.5)
            return common_1.VicinityName.DISTANT_SURROUNDINGS_SOUTHEAST;
        if (angle < 202.5)
            return common_1.VicinityName.DISTANT_SURROUNDINGS_SOUTH;
        if (angle < 247.5)
            return common_1.VicinityName.DISTANT_SURROUNDINGS_SOUTHWEST;
        if (angle < 292.5)
            return common_1.VicinityName.DISTANT_SURROUNDINGS_WEST;
        return common_1.VicinityName.DISTANT_SURROUNDINGS_NORTHWEST;
    }
}
function getVicinitiesToOffsets(bot, radii) {
    const distantRadius = radii.distantSurroundingsRadius;
    const immediateRadius = radii.immediateSurroundingsRadius;
    const origin = new vec3_1.Vec3(0, 0, 0);
    // Initialize map with vicinity names and empty arrays
    const vicinitiesToOffsets = new Map(Object.values(common_1.VicinityName).map((vicinityName) => [vicinityName, []]));
    ...
    for (let xOffset = -distantRadius; xOffset <= distantRadius; xOffset++) {
        for (let yOffset = -distantRadius; yOffset <= distantRadius; yOffset++) {
            for (let zOffset = -distantRadius; zOffset <= distantRadius; zOffset++) {
                const offset = new vec3_1.Vec3(xOffset, yOffset, zOffset);
                const vicinityNameOfOffset = classifyVicinityOfPosition(offset, origin, immediateRadius, distantRadius);
    ...
    // Pre-sort the offsets by ascending distance to origin (closest first)
    for (const [vicinityName, offsets] of vicinitiesToOffsets) {
