...
exports.getVicinitiesToDistanceSortedOffsets = getVicinitiesToDistanceSortedOffsets;
...
function classifyVicinityOfPosition(pos, botPos, immediateSurroundingsRadius, distantSurroundingsRadius) {
    ...
    if (distanceToPos <= immediateSurroundingsRadius) {
        return types_1.VicinityName.IMMEDIATE_SURROUNDINGS;
    }
        ...
        if (horizontalDist <= immediateSurroundingsRadius) {
            ...
            return pos.y > botPos.y
                ? types_1.VicinityName.DISTANT_SURROUNDINGS_UP
                : types_1.VicinityName.DISTANT_SURROUNDINGS_DOWN;
        }
        ...
        if (angle < 22.5 || angle >= 337.5)
            return types_1.VicinityName.DISTANT_SURROUNDINGS_NORTH;
        if (angle < 67.5)
            return types_1.VicinityName.DISTANT_SURROUNDINGS_NORTHEAST;
        if (angle < 112.5)
            return types_1.VicinityName.DISTANT_SURROUNDINGS_EAST;
        if (angle < 157.5)
            return types_1.VicinityName.DISTANT_SURROUNDINGS_SOUTHEAST;
        if (angle < 202.5)
            return types_1.VicinityName.DISTANT_SURROUNDINGS_SOUTH;
        if (angle < 247.5)
            return types_1.VicinityName.DISTANT_SURROUNDINGS_SOUTHWEST;
        if (angle < 292.5)
            return types_1.VicinityName.DISTANT_SURROUNDINGS_WEST;
        return types_1.VicinityName.DISTANT_SURROUNDINGS_NORTHWEST;
    }
}
const vicinitiesToDistanceSortedOffsets = null;
function getVicinitiesToDistanceSortedOffsets(bot, radii) {
    ...
    const vicinitiesToOffsets = new Map(Object.values(types_1.VicinityName).map((vicinityName) => [vicinityName, []]));
