 ...
 */
function areCoordinatesInRange(bot: Bot, coordinates: Vec3, maxDistance = 4): boolean {
...
export class PlaceBlock extends Skill {
  ...
  public async invoke(
    ...
    try {
      ...
      this.inventoryItem = this.bot.inventory.items().find(item => 
        item.name === block || 
      ...
      if (atCoordinates) {
        this.targetPosition = new Vec3(atCoordinates[0], atCoordinates[1], atCoordinates[2]);
      ...
      if (this.targetPosition) {
        return this.onResolution(new PlaceBlockResults.FailureNoAdjacentBlocks(
          block,
  ...
  private async doPlacing(): Promise<void> {
    try {
      ...
      this.isPlacing = false;
      return this.onResolution(new PlaceBlockResults.Success(
        this.blockName,
        `${this.targetPosition.x}, ${this.targetPosition.y}, ${this.targetPosition.z}`
      ));
      ...
      return this.onResolution(new PlaceBlockResults.FailureNoAdjacentBlocks(
        this.blockName,
        this.targetPosition ? `${this.targetPosition.x}, ${this.targetPosition.y}, ${this.targetPosition.z}` : "unknown"
      ));
    }
  }
}