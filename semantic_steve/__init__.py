...
class WorldState(dataclass):
    ...
    equipped: list[str]
    immediate_vicinity: dict[
        str, dict[str, list[tuple[int, int, int] | BoundingBox]]
    ]  # E.g., {"blocks": {"diorite": [(-3, 65, -3), (-3, 65, -2), ...], "chest": [(0, 66, 2)], ...}, "structures": {"snowy_weapon_smith_1": [BoundingBox(...)]}, "pois": {"grindstone": [(-3, 66, 0)]}}
    somewhat_nearby: dict[
        str, dict[str, list[str]]
    ]  # E.g., {"north": {"blocks": ["iron_bars", "lava", ...], "structures": ["snowy_farm_1", ...], "pois": [...], "biomes": ["taiga"]}, ...}
...
class SemanticSteve:
    ...
    def pathfind_to_coordinates(
        self, coordinates=(0, 64, 0), also_stop_if_found=["iron_ore", "coal_ore"]
    ) -> tuple[WorldState, Optional[ResultInfo]]:
    ...
    def search_for_thing(
        ...
        thing="stronghold",
        direction="down",
        also_stop_if_found=["chest", "diamond_ore"],
        stay_in="nether_fortress",
    ) -> tuple[WorldState, Optional[ResultInfo]]:
    ...
    def explore_for_thing(
        ...
        thing="jungle",
        direction="north",
        also_stop_if_found=["village", "ruined_portal"],
    ) -> tuple[WorldState, Optional[ResultInfo]]:
        """Pathfinds in a direction for up to approximately 300 blocks (digging/bridging as needed), stopping when..."""
        ...

    def approach_something_somewhat_nearby(
        self, something="weaponsmith", direction="north"
    ) -> tuple[WorldState, Optional[ResultInfo]]:
