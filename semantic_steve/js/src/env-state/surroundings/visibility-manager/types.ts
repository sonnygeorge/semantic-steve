...
class SphericalAngles {
   ...
   * @remarks Measured from the positive Y-axis (Minecraft's up direction),
   * where 0 is straight upward (+Y) and π is straight downward (-Y).
   */
  phi: number;
   ...
   * @remarks Measured in the XZ-plane (Minecraft's horizontal plane),
   * with 0 along the positive X-axis (east direction).
   */
  theta: number;
...
export class ThreeDimOrientation {
   ...
   */
  public angularDistanceTo(other: ThreeDimOrientation): number {
   ...
   */
  public plusAngularOffset(angularOffset: {
    theta: number;
    phi: number;
  }): ThreeDimOrientation {
    ...
    const up = new Vec3(0, 1, 0); // Y-up reference

    let thetaDirection: Vec3;
    let phiDirection: Vec3;

    // Handle special case when base vector is parallel to Y axis
    if (Math.abs(baseVec.y) > 0.999) {
      thetaDirection = new Vec3(1, 0, 0);
      phiDirection = new Vec3(0, 0, 1);
    } else {
