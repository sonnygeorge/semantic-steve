...
exports.SurroundingsHydrater = exports.BLOCKS_TO_IGNORE = exports.BOT_EYE_HEIGHT = void 0;
const vec3_1 = require("vec3");
const types_1 = require("./types");
const mineflayer_util_plugin_1 = require("@nxg-org/mineflayer-util-plugin"); // TODO: Remove this dependency
...
class SurroundingsHydrater {
    ...
    areCoordsWithinSurroundings(coords) {
     ...
     */
    getVicinityAndDistanceOfCoords(coords) {
        const botCoords = this.bot.entity.position;
        const distanceToCoords = botCoords.distanceTo(coords);
        ...
        if (distanceToCoords <= this.radii.immediateSurroundingsRadius) {
            return [types_1.Vicinity.IMMEDIATE_SURROUNDINGS, distanceToCoords];
        }
        ...
        if (dxz <= this.radii.immediateSurroundingsRadius) {
            if (coords.y > botCoords.y)
                return [types_1.Vicinity.DISTANT_SURROUNDINGS_UP, distanceToCoords];
            if (coords.y < botCoords.y)
                return [types_1.Vicinity.DISTANT_SURROUNDINGS_DOWN, distanceToCoords];
        }
        ...
        if (angle >= 337.5 || angle < 22.5)
            return [types_1.Vicinity.DISTANT_SURROUNDINGS_NOTH, distanceToCoords];
        if (angle >= 22.5 && angle < 67.5)
            return [types_1.Vicinity.DISTANT_SURROUNDINGS_NORTHEAST, distanceToCoords];
        if (angle >= 67.5 && angle < 112.5)
            return [types_1.Vicinity.DISTANT_SURROUNDINGS_EAST, distanceToCoords];
        if (angle >= 112.5 && angle < 157.5)
            return [types_1.Vicinity.DISTANT_SURROUNDINGS_SOUTHEAST, distanceToCoords];
        if (angle >= 157.5 && angle < 202.5)
            return [types_1.Vicinity.DISTANT_SURROUNDINGS_SOUTH, distanceToCoords];
        if (angle >= 202.5 && angle < 247.5)
            return [types_1.Vicinity.DISTANT_SURROUNDINGS_SOUTHWEST, distanceToCoords];
        if (angle >= 247.5 && angle < 292.5)
            return [types_1.Vicinity.DISTANT_SURROUNDINGS_WEST, distanceToCoords];
        if (angle >= 292.5 && angle < 337.5)
            return [types_1.Vicinity.DISTANT_SURROUNDINGS_NORTHWEST, distanceToCoords];
        return [types_1.Vicinity.DISTANT_SURROUNDINGS_NOTH, distanceToCoords];
    }
    isBlockVisible(block, blockCoords) {
        ...
        if (numExposedSides === 0)
            return false;
        // Raycast to check if the block is visible
        const eyePosition = this.bot.entity.position.offset(0, exports.BOT_EYE_HEIGHT, 0);
        const bbs = block.shapes.map((s) => mineflayer_util_plugin_1.AABB.fromShape(s, blockCoords));
        for (const bb of bbs) {
            ...
            for (const vertex of vertices) {
                ...
                if (hit != null && hit.position) {
                    if (hit.position.equals(blockCoords))
                        return true;
                }
            }
        }
    ...
    *getAllChunksInSurroundings() {
        // Iterate through loaded chunks
        for (const chunkCoordsAndColumn of this.bot.world.getColumns()) {
            if (!chunkCoordsAndColumn.column)
                continue;
            // Skip this chunk if none of its corners are within surroundings
            const chunkMinX = chunkCoordsAndColumn.chunkX << 4;
            const chunkMinY = this.bot.game.minY; // Bottom-most y-level of world
            const chunkMinZ = chunkCoordsAndColumn.chunkZ << 4;
            const chunkMaxX = chunkMinX + 16;
            const chunkMaxY = this.bot.game.height; // World height
            const chunkMaxZ = chunkMinZ + 16;
            const chunkCornerCoords = [
                new vec3_1.Vec3(chunkMinX, chunkMinY, chunkMinZ),
                new vec3_1.Vec3(chunkMinX, chunkMinY, chunkMaxZ),
                new vec3_1.Vec3(chunkMaxX, chunkMinY, chunkMinZ),
                new vec3_1.Vec3(chunkMaxX, chunkMinY, chunkMaxZ),
                new vec3_1.Vec3(chunkMinX, chunkMaxY, chunkMinZ),
                new vec3_1.Vec3(chunkMinX, chunkMaxY, chunkMaxZ),
                new vec3_1.Vec3(chunkMaxX, chunkMaxY, chunkMinZ),
                new vec3_1.Vec3(chunkMaxX, chunkMaxY, chunkMaxZ),
            ];
            if (!chunkCornerCoords.some((cornerCoords) => this.areCoordsWithinSurroundings(cornerCoords)))
                continue;
            // Yield since chunk is at least partially in surroundings
            yield chunkCoordsAndColumn;
        }
    }
    *getAllBlocksFromChunkThatAreInSurroundings(chunkCoordsAndColumn) {
        const cursor = new vec3_1.Vec3(0, 0, 0);
        for (cursor.y = this.bot.game.height; cursor.y >= this.bot.game.minY; cursor.y--) {
            for (cursor.x = 0; cursor.x < 16; cursor.x++) {
                for (cursor.z = 0; cursor.z < 16; cursor.z++) {
                    const blockCoords = new vec3_1.Vec3((chunkCoordsAndColumn.chunkX << 4) + cursor.x, cursor.y, (chunkCoordsAndColumn.chunkZ << 4) + cursor.z);
                    if (!this.areCoordsWithinSurroundings(blockCoords)) {
                        continue;
                    }
                    const block = chunkCoordsAndColumn.column.getBlock(cursor);
                    if (block.name in exports.BLOCKS_TO_IGNORE)
                        continue;
    ...
    *getAllVisibleBlocksInSurroundings() {
        for (const chunkCoordsAndColumn of this.getAllChunksInSurroundings()) {
            for (const { block, blockCoords, } of this.getAllBlocksFromChunkThatAreInSurroundings(chunkCoordsAndColumn)) {
                if (!this.isBlockVisible(block, blockCoords))
                    continue;
                yield { block, blockCoords };
            }
        }
    ...
    getHydration() {
        ...
        const surroundings = new types_1._Surroundings(this.bot, this.radii);
        const biomesToClosestDistanceByDirection = new Map();
        Object.values(types_1.Direction).forEach((dir) => {
            biomesToClosestDistanceByDirection.set(dir, new Map());
        });
        const blocksToClosestDistanceByDirection = new Map();
        Object.values(types_1.Direction).forEach((dir) => {
            blocksToClosestDistanceByDirection.set(dir, new Map());
        });
        // Iterate through visible blocks in surroundings
        for (const { block, blockCoords, } of this.getAllVisibleBlocksInSurroundings()) {
            const [blockVicinity, blockDistance] = this.getVicinityAndDistanceOfCoords(blockCoords);
            const biome = block.biome.id;
            if (blockVicinity === types_1.Vicinity.IMMEDIATE_SURROUNDINGS) {
                // Add block
                ...
                // Add biome
                if (biome !== -1) {
                    surroundings.immediate.biomes.add(biome);
                }
            }
            else {
                ...
                const direction = blockVicinity;
                const distantSurroundingsInDirection = surroundings.distant.get(direction);
                // Update blocks count
                const currentCount = distantSurroundingsInDirection.blocksToCounts.get(block.name) || 0;
                distantSurroundingsInDirection.blocksToCounts.set(block.name, currentCount + 1);
                // Update closest block if the block is the new closest of this block for this direction
                const currentBlockDistance = blocksToClosestDistanceByDirection.get(direction).get(block.name) ||
                    Infinity;
                if (blockDistance < currentBlockDistance) {
                    blocksToClosestDistanceByDirection
                        .get(direction)
                        .set(block.name, blockDistance);
                    distantSurroundingsInDirection.blocksToClosestCoords.set(block.name, blockCoords);
                }
                // Update closest biome if the block's biome is the new closest of this biome for this direction
                if (biome !== -1) {
                    const currentBiomeDistance = biomesToClosestDistanceByDirection.get(direction).get(biome) ||
                        Infinity;
                    if (blockDistance < currentBiomeDistance) {
                        biomesToClosestDistanceByDirection
                            .get(direction)
                            .set(biome, blockDistance);
                        distantSurroundingsInDirection.biomesToClosestCoords.set(biome, blockCoords);
                    }
                }
            }
        }
...
exports.SurroundingsHydrater = SurroundingsHydrater;
