...
class Surroundings {
    constructor(bot, radii) {
        ...
        // Private
        this.bot = bot;
        this.eagerlyProcessedBlocks = new cache_1.BlocksCache(bot);
    ...
    doEagerBlockProcessing(block) {
        ...
        if (!block || exports.BLOCKS_TO_IGNORE.includes(block.name))
            return;
        // 1. Add to cache
        this.eagerlyProcessedBlocks.set(cache_1.BlocksCache.getKeyFromVec3(block.position), block);
        // 2. If in a vicinity and visible, add to that vicinity
        this.classifyAndAddBlockToVicinityIfIsVisibleInOne(block);
    }
    removeBlockFromEagerlyProcessed(block) {
    ...
    handleChunkLoad(chunkPos) {
        ...
        (0, chunk_1.applyFuncToCoordsInChunk)(this.bot, this.doEagerBlockProcessing.bind(this), chunkPos);
    }
    handleChunkUnload(chunkPos) {
        (0, chunk_1.applyFuncToCoordsInChunk)(this.bot, this.removeBlockFromEagerlyProcessed.bind(this), chunkPos);
    }
    ...
    handleBotMovement() {
        ...
        const curInRadiusCoords = new Set();
        const alreadyCheckedVisibility = new Set();
        for (const coord of (0, misc_1.getAllCoordsWithinRadiusToPos)(curBotPosition, this.radii.distantSurroundingsRadius, this.bot)) {
            const key = cache_1.BlocksCache.getKeyFromVec3(coord);
        ...
        for (const coord of (0, misc_1.getAllCoordsWithinRadiusToPos)(prevBotPosition, this.radii.distantSurroundingsRadius, this.bot)) {
            const key = cache_1.BlocksCache.getKeyFromVec3(coord);
            if (!curInRadiusCoords.has(key)) {
                this.removeBlockFromEagerlyProcessed(key);
            }
        }
        const reaccountForVisibilities = (visibleVicinityContents) => {
            for (const [key, block] of visibleVicinityContents.blocksLookup) {
                if (alreadyCheckedVisibility.has(key))
                    continue;
                const blockIsVisible = (0, visibility_1.isBlockVisible)(this.bot, block, cache_1.BlocksCache.getVec3FromKey(key));
                if (blockIsVisible) {
