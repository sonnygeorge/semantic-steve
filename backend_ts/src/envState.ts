...
export class Surroundings {
  ...
  private observeSurroundings(): void {
    ...
    for (const {block, column} of this.getAllBlocks(this.distantRadius)) {        
        ...
        // TODO: (question from Sonny) Why do we need to this? Why can't we use block.biome?
...
//   }
  ...
  public getReadableString(): string {
    ...
    output += `Hunger: ${this.hunger}\n`;
    output += `Time of Day: ${this.timeOfDay}\n`;
    // output += `Biome: ${this.biome}\n`;
    output += `Inventory: ...\n`;  // TODO
    output += `Equipped: ...\n`;  // TODO
    output += `Notepad: ...\n`;  // TODO
    
    
    // TODO: This is TERRIBLY manual, but fine for now?
    // TODO: Output biome names instead of IDs!
    output += `Surroundings:\n{`;
    if (this.surroundings.lastObservedImmediateSurroundings!== null) {
        output += `\n   "immediateSurroundings": {`;
        
        // Blocks
        let immediateBlocksString = "";
        for (const [blockType, coords] of this.surroundings.lastObservedImmediateSurroundings.blocks!.entries()) {
            const coordsString = coords.map(c => `(${c.x},${c.y},${c.z})`).join(",\n            ");
            immediateBlocksString += `\n         "${blockType}": [\n            ${coordsString},\n         ],`;
        }
        output += `\n      "blocks": {${immediateBlocksString}`;

        // Biomes
        if (this.surroundings.lastObservedImmediateSurroundings.biomes !== null) {
          const immediateBiomesString = Array.from(this.surroundings.lastObservedImmediateSurroundings.biomes).join(",\n        ");
          output += `\n      },\n      "biomes": [\n            ${immediateBiomesString}\n      ],`;
        }
    ...
    if (this.surroundings.lastObservedDistantSurroundings !== null) {
        ...
        for (const [direction, data] of this.surroundings.lastObservedDistantSurroundings.entries()) {
            ...
            // Blocks
            let distantBlocksString = "";
            for (const [blockType, count] of data.blocksToCounts!.entries()) {
                distantBlocksString += `\n            "${blockType}": ${count},`;
            }
            if (distantBlocksString === "") {
                output += '\n         "blocks": null,';
            }
            else {
                output += `\n         "blocks": {${distantBlocksString}\n         },`;
            }

            // Biomes
            let distantBiomesString = "";
            for (const [biome, coords] of data.biomesToClosestCoords!.entries()) {
                distantBiomesString += `\n            "${biome}": (${coords.x},${coords.y},${coords.z}),`;
            }
            if (distantBiomesString === "") {
                output += '\n         "biomes": null,';
            }
            else {
                output += `\n         "biomes": {${distantBiomesString}\n         },`;
            }
