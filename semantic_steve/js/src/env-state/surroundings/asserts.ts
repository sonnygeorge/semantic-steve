...
export function assertMinimumRaycastDensity(
  ...
  if (nRaycastOrientations < minimumRaysRequired) {
    ...
    throw new Error(
      `Need ${minimumRaysRequired} rays for radius ${radiusOfInterest}, have ${nRaycastOrientations}. ` +
        `Current ray spacing: ${approximateRaySpacing.toFixed(2)} blocks.`
    );
  }

  console.log(
    `✓ ${nRaycastOrientations} rays sufficient for ${Math.ceil(
      blocksOnSurface
    )} blocks at radius ${radiusOfInterest}`
  );
}
