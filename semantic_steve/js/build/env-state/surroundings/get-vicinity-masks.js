...
exports.getVicinityMasks = getVicinityMasks;
...
function getVicinityMasks(bot, radii) {
    const vicinityMasks = new Map();
    for (const vicinityName of Object.values(common_1.VicinityName)) {
        vicinityMasks.set(vicinityName, new voxel_space_around_bot_eyes_1.VoxelSpaceAroundBotEyes(bot, radii.distantSurroundingsRadius, false // Default value for the mask
        ));
    }
    const origin = new vec3_1.Vec3(0, 0, 0);
    for (const offset of vicinityMasks
        .get(common_1.VicinityName.IMMEDIATE_SURROUNDINGS)
        .iterAllOffsets()) {
        const vicinityNameOfOffset = classifyVicinityOfPosition(offset, origin, radii.immediateSurroundingsRadius, radii.distantSurroundingsRadius);
