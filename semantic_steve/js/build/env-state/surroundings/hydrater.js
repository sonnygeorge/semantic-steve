...
class SurroundingsHydrater {
    ...
    getVicinityForPosition(pos) {
        ...
        if (distance <= this.radii.immediateSurroundingsRadius) {
            return types_1.Vicinity.IMMEDIATE_SURROUNDINGS;
        }
        // Check if outside of distant surroundings
        if (distance > this.radii.distantSurroundingsRadius) {
            return types_1.Vicinity.IMMEDIATE_SURROUNDINGS; // Default fallback, will be filtered later
        }
        ...
        if (horizontalDist <= this.radii.immediateSurroundingsRadius) {
            return pos.y > botPos.y ? types_1.Vicinity.DISTANT_SURROUNDINGS_UP : types_1.Vicinity.DISTANT_SURROUNDINGS_DOWN;
        }
        // Determine direction based on angle
        const angle = ((Math.atan2(pos.x - botPos.x, botPos.z - pos.z) * 180) / Math.PI + 360) % 360;
        if (angle < 22.5 || angle >= 337.5)
            return types_1.Vicinity.DISTANT_SURROUNDINGS_NORTH;
        if (angle < 67.5)
            return types_1.Vicinity.DISTANT_SURROUNDINGS_NORTHEAST;
        if (angle < 112.5)
            return types_1.Vicinity.DISTANT_SURROUNDINGS_EAST;
        if (angle < 157.5)
            return types_1.Vicinity.DISTANT_SURROUNDINGS_SOUTHEAST;
        if (angle < 202.5)
            return types_1.Vicinity.DISTANT_SURROUNDINGS_SOUTH;
        if (angle < 247.5)
            return types_1.Vicinity.DISTANT_SURROUNDINGS_SOUTHWEST;
        if (angle < 292.5)
            return types_1.Vicinity.DISTANT_SURROUNDINGS_WEST;
        return types_1.Vicinity.DISTANT_SURROUNDINGS_NORTHWEST;
    }
    processChunk(chunkX, chunkZ) {
        ...
        if (minDist > maxDistance + this.radii.distantSurroundingsRadius)
            return; // Skip chunk if even the closest point is too far
        // Process blocks in the chunk
        const minY = (_a = this.bot.game.minY) !== null && _a !== void 0 ? _a : 0;
        const maxY = (_b = this.bot.game.height) !== null && _b !== void 0 ? _b : 256;
        for (let x = 0; x < 16; x++) {
            for (let z = 0; z < 16; z++) {
                ...
                if (horizontalDist > maxDistance + 1)
                    continue;
    ...
    updateBlock(block) {
        ...
        if (exports.BLOCKS_TO_IGNORE.includes(block.name)) {
    ...
    addBlockToVicinity(block, vicinity) {
        const pos = block.position;
        if (vicinity === types_1.Vicinity.IMMEDIATE_SURROUNDINGS) {
    ...
    removeBlockFromVicinity(blockName, pos, vicinity) {
        if (vicinity === types_1.Vicinity.IMMEDIATE_SURROUNDINGS) {
    ...
    processItemEntity(entity) {
        return __awaiter(this, void 0, void 0, function* () {
            // await entityUpdate due to not yet receiving metadata
            if (entity.metadata == null || entity.metadata.length === 0) {
    ...
    addItemToVicinity(itemName, pos, vicinity, entityId) {
        if (vicinity === types_1.Vicinity.IMMEDIATE_SURROUNDINGS) {
            // Add to immediate surroundings
    ...
    removeItemFromVicinity(itemName, entityId, vicinity) {
        if (vicinity === types_1.Vicinity.IMMEDIATE_SURROUNDINGS) {
    ...
    recalculateVicinities() {
        ...
        for (const [blockKey, data] of this.blockLookup.entries()) {
            ...
            const pos = new vec3_1.Vec3(x, y, z);
